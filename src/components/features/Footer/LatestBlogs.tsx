import { Icon } from "@iconify/react";
import blogsItem, { type BlogsItem } from "../../../types/BlogsItem";
import { useEffect, useState } from "react";

const LatestBlogs = () => {
  const [latestBlogs, setLatestBlogs] = useState<BlogsItem[]>([]);
  useEffect(() => {
    const blogs = blogsItem.filter((blog) => blog.id > 4);
    setLatestBlogs(blogs);
  }, [blogsItem]);
  return (
    <div className="w-70 xs:w-80 sm:w-65">
      <h3 className="text-2xl font-bold relative before:w-20 before:h-1 before:bg-indigo-500 before:absolute before:-bottom-1 before:left-0.5 mb-8">
        Latest Blogs
      </h3>
      <div className="flex flex-col gap-4">
        {latestBlogs.map((blog) => (
          <div className="flex gap-2 group" key={blog.id}>
            <img
              src={blog.img}
              alt="blog"
              className="size-16 object-cover rounded-md"
            />
            <div className="cursor-pointer">
              <h3 className="font-semibold text-sm line-clamp-2 text-ellipsis group-hover:underline">
                {blog.title}
              </h3>
              <p className="text-[0.7em] text-[grey] mt-1 flex items-center gap-1">
                <Icon icon="ri:calendar-line" /> {blog.create_at}
              </p>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};

export default LatestBlogs;
